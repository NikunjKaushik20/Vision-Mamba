...
\begin{tcolorbox}[
  ...
  {\large\bfseries\color{AccentCyan} $\blacktriangleright$\enspace Live Demo Video}\\[0.5em]
  {\normalsize\color{white}
    Watch the full system demonstration — YOLO-based fracture localisation,\\
    heatmap visualisation, and real-time inference in action.
  }\\[0.7em]
  {\small\color{TealBlue}
    \href{https://youtu.be/xBkgo6VmUOo}{\textbf{\textcolor{AccentCyan}{\underline{https://youtu.be/xBkgo6VmUOo}}}}
  }
\end{tcolorbox}
...
YOLO-based localisation stage is used only to identify the suspected fracture region prior to
classification; all quantitative results reported in this document correspond to the
\textbf{FractureMamba-ViT} classifier. A
...
In the full workflow, a lightweight YOLO model is used strictly as a localisation aid to
highlight the suspected fracture region. The evaluation tables, discussion, and conclusions in
this report refer only to the output of the FractureMamba-ViT classifier.
...
\begin{enumerate}[label=\textbf{Stage \arabic*.}, leftmargin=*, itemsep=6pt]
  ...
  A YOLO-based localisation step first identifies the likely fracture region. The localised image
  is then passed to the FractureMamba-ViT dual-stream backbone described in Section~4, and only
  the classifier's output is used for the metrics reported in this document.
...
\begin{itemize}
  ...
  Refining the existing localisation stage into a calibrated bounding-box module would enable
  more precise fracture outlining, supporting surgical planning and automated radiology reporting.
